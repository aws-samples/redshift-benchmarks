...
#variables only used in this script
...
setup_user()
{
	current_user=$(rsql -D ${DSN} -t -A -v ON_ERROR_STOP=1 -c "select current_user;")

	echo "rsql -D ${DSN} -t -a -v ON_ERROR_STOP=1 -c \"alter user \\\"${current_user}\\\" set enable_result_cache_for_session = false;\""
	rsql -D ${DSN} -t -a -v ON_ERROR_STOP=1 -c "alter user \"${current_user}\" set enable_result_cache_for_session = false;"

	echo "rsql -D ${DSN} -v ON_ERROR_STOP=1 -q -A -t -c \"ALTER USER ${current_user} SET search_path=${SCHEMA_NAME},public;\""
	rsql -D ${DSN} -v ON_ERROR_STOP=1 -q -A -t -c "ALTER USER ${current_user} SET search_path=${SCHEMA_NAME},public;"
}
